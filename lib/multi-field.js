    ...
    b.validate = function (form, callback) {
      Promise.all(b.fieldsList.map((row, i) => {
        return new Promise(resolve => {
          row.validate(form, (_err, boundField) => {
  ...
  f.toHTML = function (name, iterator) {
    ...
    return tag(
      ...
      (
        ...
        tag('div', { classes: ['field__multi-row-container'].concat(cssClasses.rowContainer || []), 'data-multi-row-prefix': name }, value, true)
      ),
      true
    );
  };
